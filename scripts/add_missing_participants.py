#!/usr/bin/env python3
"""
Script to add missing participants from both datasets
"""
import json
import csv
from pathlib import Path

# Missing participants from Code Combat event
missing_code_combat = """NITHISH KUMAR SARAVANAN	nithishkumarsaravanan8@gmail.com	8015413019	Saranathan college of engineering
...
# Previous data that might have missing participants
previous_data_raw = """Ranjhani. NR|iammeranjnr@gmail.com|8838132006|Saranathan College of Engineering
...
Sudharshini|sudharshini22102007@gmail.com|8754235645|Srm University
...
def main():
    ...
    missing_cc = parse_tab_data(missing_code_combat)
    previous = parse_pipe_data(previous_data_raw)

    # Combine all new entries
    all_new_entries = missing_cc + previous
    ...
    existing_emails = {entry['email'].lower() for entry in data}
    ...
    print(f"\nProcessing {len(unique_entries)} potential new participants...")
    print(f"Starting ID: {next_id}")

    for new_entry in unique_entries:
        ...
        if email not in existing_emails:
            ...
            # Add Code Combat event if from missing_cc list
            if any(e['email'] == email for e in missing_cc):
                if 'Code Combat' not in str(new_record['events']):
                    new_record['events'].append('Code Combat')
            ...
            print(f"  + Added: {new_record['name']} ({email})")

    # Save updated data
    with open(json_path, 'w') as f:
        json.dump(data, f, indent=2)

    print(f"\n✓ Added {added_count} new participants")
